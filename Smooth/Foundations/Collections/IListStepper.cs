...
namespace Smooth.Collections {
	...
	/// </summary>
	public class IListStepperWithIndex<T> : IEnumerable<System.Tuple<T, int>> {
		private readonly IList<T> list;
		private readonly int startIndex;
		private readonly int step;
		...
		public IEnumerator<System.Tuple<T, int>> GetEnumerator() {
			for (int i = startIndex; 0 <= i && i < list.Count; i += step) {
				yield return new System.Tuple<T, int>(list[i], i);
			}
		}
